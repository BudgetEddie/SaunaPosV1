// ============================================================================
// THE MENU — every section and every item, as code.
//
// WHAT IT IS
//   The menu used to exist only as rows typed into one till. That made it
//   unbacked-up, unreviewable, and impossible to put on a second box without
//   retyping seventy-five items by hand. This file is the menu written down,
//   so it can be recreated anywhere the app runs.
//
//   LOCAL-FIRST NOTE: the catalogue is deliberately unscoped — no siteId, see
//   the note above `model Category` in schema.prisma. Each site runs its own
//   box with its own database file, so the menu on this box IS this site's
//   menu. There is nothing to pick between.
//
// ✅ SAFE TO RE-RUN, and safe on a till that is already trading.
...
//
// HOW TO RUN IT
//   On this laptop:  cd server && npx ts-node prisma/seed-menu.ts
//   On a site's box: docker compose -f docker-compose.prod.yml exec app \
//                      npx ts-node prisma/seed-menu.ts
//
// WHERE IT'S USED
//   Nowhere in the running app. A command run by hand when setting up a box,
//   or to top up a menu that is missing a section.
// ============================================================================
...
async function main() {
  ...
  for (const section of MENU) {
    ...
    // Category names are unique, so this is the same test the database applies.
    let category = await prisma.category.findFirst({ where: { name: section.name } });
